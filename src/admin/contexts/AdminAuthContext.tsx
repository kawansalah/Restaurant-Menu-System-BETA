  ...
  const login = async (credentials: LoginCredentials): Promise<boolean> => {
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
        email: credentials.username.includes('@') ? credentials.username : '', // Use email if provided
      ...
      if (!credentials.username.includes('@')) {
        ...
        const { data: authDataByEmail, error: authErrorByEmail } = await supabase.auth.signInWithPassword({
      ...
      const { success: loginUpdateSuccess } = await updateLastLogin(adminUser.id);
      ...
      const sessionToken = `admin-${adminUser.id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
