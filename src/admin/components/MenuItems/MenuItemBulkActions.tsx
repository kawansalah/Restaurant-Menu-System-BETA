...
interface MenuItemBulkActionsProps {
  selectedItems: AdminMenuItem[];
  allItems: AdminMenuItem[];
  onSelectionChange: (items: AdminMenuItem[]) => void;
  onActionComplete: () => void;
}

const MenuItemBulkActions: React.FC<MenuItemBulkActionsProps> = ({
  selectedItems,
  onSelectionChange,
  onActionComplete,
}) => {
  ...
  const getText = (key: string) => {
    const texts = {
      ...
      confirmDelete: {
        en: "Are you sure you want to delete the selected menu items? This action cannot be undone.",
        ar: "هل أنت متأكد من أنك تريد حذف عناصر القائمة المحددة؟ لا يمكن التراجع عن هذا الإجراء.",
        ku: "دڵنیای کە دەتەوێت خواردنە هەڵبژراوەکان بسڕیتەوە؟ ئەم کردارە ناگەڕێتەوە.",
      },
      cancel: {
        en: "Cancel",
        ar: "إلغاء",
  ...
  const executeBulkAction = async (action: string) => {
    ...
    try {
      switch (action) {
        case "delete":
          const deleteResult = await deleteMenuItems(selectedIds);
          if (deleteResult.success) {
            alerts.showSuccess(
              `${deleteResult.deletedCount} menu items deleted successfully`,
              "Items deleted"
            );
            onSelectionChange([]);
            onActionComplete();
        ...
        case "make_available":
          const availableResult = await bulkToggleMenuItemAvailability(
            selectedIds,
            true
          );
          if (availableResult.success) {
            alerts.showSuccess(
              `${availableResult.updatedCount} items made available`,
              "Items updated"
            );
            onActionComplete();
          } else {
            alerts.showError(
              availableResult.error || "Failed to update items",
              "Update failed"
            );
          }
          break;
        ...
        case "make_unavailable":
          const unavailableResult = await bulkToggleMenuItemAvailability(
            selectedIds,
            false
          );
          if (unavailableResult.success) {
            alerts.showSuccess(
              `${unavailableResult.updatedCount} items made unavailable`,
              "Items updated"
            );
            onActionComplete();
          } else {
            alerts.showError(
              unavailableResult.error || "Failed to update items",
              "Update failed"
            );
          }
          break;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-2">
            ...
            >
              ...
              <span className="hidden xs:inline sm:inline">{getText("makeAvailable")}</span>
            </button>
            ...
            >
              ...
              <span className="hidden xs:inline sm:inline">{getText("makeUnavailable")}</span>
            </button>
...
export default MenuItemBulkActions;
