...
const PaginatedSubCategoryList: React.FC<PaginatedSubCategoryListProps> = ({
  ...
  const loadInitialSubCategories = async () => {
    try {
      ...
      // Fetch all subcategories with filters
      const { data, error: fetchError } = await fetchSubCategories(filters);
      ...
      if (fetchError) {
          ...
          filteredData = filteredData.filter((subcategory) => {
            ...
            return (
              ...
              subcategory.label_ar
                ...
                .toLowerCase()
                .includes(searchTerm) ||
              subcategory.label_en?.toString().toLowerCase().includes(searchTerm)
            );
          });
        }

        // Apply column filters (excluding specific IDs as they're already handled by service)
        Object.entries(filters).forEach(([key, value]) => {
          if (key === 'category_id' || key === 'restaurant_id' || !value || !value.trim()) return;
        ...
        // Set all subcategories (no pagination)
  ...
  const displayedSubCategories = showAllItems
    ? subcategories
  ...
  if (error) {
    return (
      <div className={`${className}`}>
        <div
          className={`${theme.bgCard} rounded-3xl p-6 border ${theme.borderSubCategory} text-center`}
        >
  ...
  if (loading) {
    return (
      <div className={`${className}`}>
        <div
          className={`${theme.bgCard} rounded-3xl p-4 border ${theme.borderSubCategory}`}
        >
          {/* Header */}
    ...
    <div className={`${className}`}>
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          >
            {getText("showing")} {displayedSubCategories.length}
            {subcategories.length > displayedSubCategories.length &&
              ` ${getText("of")} ${subcategories.length}`}
          </div>
        </div>
        ...
        {/* See More Button */}
        {!showAllItems && subcategories.length > INITIAL_DISPLAY_LIMIT && (
