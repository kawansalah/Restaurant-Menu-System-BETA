...
const Loader: React.FC<LoaderProps> = ({ onLoadComplete, duration = 1500 }) => {
  ...
  const currentName =
    SystemName[language as keyof typeof SystemName] || SystemName.en;
  const currentSubtitle =
    subtitleTexts[language as keyof typeof subtitleTexts] || subtitleTexts.en;
  const currentSteps =
    stepTexts[language as keyof typeof stepTexts] || stepTexts.en;
  ...
  useEffect(() => {
    ...
    const progressDuration = duration / 1000 - 1;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {loadingProgress}
            {defaultMenuConfig.ui?.loader.percentage[
              language as keyof typeof defaultMenuConfig.ui.loader.percentage
            ] || "%"}
          </div>
        </div>
        ...
        {/* Loading Steps */}
        <div ref={stepsRef} className="w-full max-w-xs space-y-3">
          {currentSteps.map((step, index) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  background:
                    index <= currentStep ? "var(--bg-main)" : "transparent",
                  color:
                    index <= currentStep
                      ? theme === "dark"
                        ? "var(--button-text-primary)"
